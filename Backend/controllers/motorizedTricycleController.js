...
exports.updateStatus = async (req, res) => {
  try {
    ...
    await MotorizedTricycle.updateStatus(id, status);
    res.json({ message: "Motorized Tricycle status updated successfully" });
  } catch (err) {
    res.status(500).json({ status: "error", error: err.message });
  }
};
...
exports.updateRemarks = async (req, res) => {
  try {
    ...
    await MotorizedTricycle.updateRemarks(id, status, remarks);
    res.json({
      message: "Motorized Tricycle remarks and  status updated successfully",
    });
  } catch (err) {
    res.status(500).json({ status: "error", error: err.message });
  }
